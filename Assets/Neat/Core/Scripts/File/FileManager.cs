...
namespace Neat.GameManager
{
    public class FileManager : MonoBehaviour
    {
        ...
        public static bool Serialize(object file, string path)
        {
            try
            {
                SerializeBinary(file, path);
                Debug.Log("Successfully saved " + file.GetType().ToString() + " to " + path);
                return true;
            }
            catch (Exception e)
            {
                Debug.LogError("Failed to saved " + file.GetType().ToString() + " to " + path
                    + "\n" + e.Message);
                return false;
            }
        }
        public static bool DeserializeBinary<T>(out T file, string path, bool log = true)
        {
            try
            {
                ...
                if (log) Debug.Log("Successfully loaded " + file.GetType().ToString() + " from " + path);
                return true;
            ...
            catch (Exception e)
            {
                ...
                Debug.LogError("Failed to load " + typeof(T).ToString() + " from " + path
                    + "\n" + e.Message);
                return false;
            }
        }
